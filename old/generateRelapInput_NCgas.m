...
%% INPUT----------------------------------------------------
    ...
    condenser_length=1;    
    adiabatic_length=0.3;
    tube_inner_diam=0.021;
    tube_outer_diam=0.03;
    emptyVol=pi*tube_inner_diam*(condenser_length+adiabatic_length);
    ...
    amount_of_adiabatic_parts=15;
    amount_of_condensation_parts=50;
...
%% Pre-test calculations
    ...
    % num2str is important when writing values with fprintf
        ...
        adiabatic_part_length=num2str(adiabatic_length/amount_of_adiabatic_parts);
        pipe_part_length=num2str(condenser_length/amount_of_condensation_parts);
        radius_inner=num2str(tube_inner_diam/2);
        radius_outer=num2str(tube_outer_diam/2);
        cross_section=num2str(pi*(tube_inner_diam/2)^2);
   ...
   %change part numbering to strings
       ...
       condensation_minus_one=num2str(amount_of_condensation_parts-1);
       amount_of_condensation_parts=num2str(amount_of_condensation_parts);
                ...
                for Heatflux_count=1:amountHeatflux
                    ...
                    for Superheat_count=1:amountSuperheat
                        ...
                        for Mflowss_count=1:amountMflowss
                            ...
                            Tps = IAPWS_IF97('Tsat_p',Pps(Pps_count)/1e6*(1-NC(NC_count)));
                            Theater=num2str(Tps-2);
                            ...
                            NC_mole_fr=NC_gas;
                            NC_gas(NC_gas=='.')='';
                            Helium_content=num2str(Helium(NC_count));
                            Helium_content(Helium_content=='.')='';

                            %enthalpies from temperature
                            %from IAPWS 97 by Mark Mikofski (this only calculates enthalpies)
%                                 uLsat = num2str(1000*IAPWS_IF97('hL_p',Pps(a)/1e6)); % [J/kg] saturated liquid enthalpy, input in MPa
%                                 uVsat = num2str(1000*IAPWS_IF97('hV_p',Pps(a)/1e6)); % [J/kg] saturated vapor enthalpy, input in MPa
                            %from Xsteam
                            uLsat = num2str(1000*XSteam('uL_p',Pps(Pps_count)/1e5*(1-NC(NC_count)))); % [J/kg] saturated liquid enthalpy, input in Bar
                            uVsat = 1000*XSteam('uV_p',Pps(Pps_count)/1e5*(1-NC(NC_count))); % [J/kg] saturated vapor enthalpy, input in Bar
%                                 uVsat_N=5/2*8.31*Tps(b)/28*1000;
%                                 uVsat=uVsat*(1-NC(d))+uVsat_N*NC(d);
                            uVsat=num2str(uVsat);
                            ...
                            %NC content 
                            Helium_fraction=NC(NC_count)*Helium(NC_count);
                            Nitrogen_fraction=1-Helium_fraction;
                            Helium_fraction=num2str(Helium_fraction);
                            Nitrogen_fraction=num2str(Nitrogen_fraction);
                            if isempty(strfind(Helium_fraction,'.'))
                                Helium_fraction(end+1)='.';
                            end
                            if isempty(strfind(Nitrogen_fraction,'.'))
                                Nitrogen_fraction(end+1)='.';
                            end
                            ...
                            fileName=strcat(PrimaryPressure_withoutdots,'_',PrimaryTemp_name,'_',NC_gas,'_',Helium_content,'_',Heat_flux_withoutdots,'_',Superheat_name,'_',Mflow_secondary_hours);                      %save file name WITHOUT extension
                            filePath=strcat(dirInput,PrimaryPressure_withoutdots,'_',PrimaryTemp_name,'_',NC_gas,'_',Helium_content,'_',Heat_flux_withoutdots,'_',Superheat_name,'_',Mflow_secondary_hours,'.i'); %create file path for current combination
                            ...
                            fprintf(fid,'115 %s      %s\n',Helium_fraction,Nitrogen_fraction);                                   %****************************************************
                            fprintf(fid,'*================================================================\n');
                            ...
                            fprintf(fid,'201 %s       %s      %s      0        %s     %s       1200000\n', endtime, mindt, maxdt, minor, major); %****************************************************
                            fprintf(fid,'*================================================================\n');
...
%                                 fprintf(fid,'1101201  004  %s.      %s.          0.01  0. 0.   %s \n',PrimaryPressure, PrimaryTemp, amount_of_heater_tube_parts); %***************************************************
                            ...
                            fprintf(fid,'1200101  110100002  121010001  %s    0.    0.     00000000\n', heater_cross_section);    %*************************************************************
                            ...
                            %******************* adiabatic part (now also a tube)

                            fprintf(fid,'*----------------------------------------------------------------\n');
                            fprintf(fid,'* component 121 - adiabatic part of the pipe\n');
                            fprintf(fid,'*        name   type\n');
                            fprintf(fid,'1210000  pipe  pipe\n');
                            fprintf(fid,'*        no.volumes\n');
                            fprintf(fid,'1210001  %s \n', amount_of_adiabatic_parts);  
                            fprintf(fid,'*        area                             vol.no.\n');
                            fprintf(fid,'1210101  %s                               %s \n', cross_section, amount_of_adiabatic_parts);
                            fprintf(fid,'*        length                           vol.no.\n');
                            fprintf(fid,'1210301  %s                               %s \n', adiabatic_part_length, amount_of_adiabatic_parts);
                            fprintf(fid,'*        v-ang                            vol.no.\n');
                            fprintf(fid,'1210601  90.                              %s \n', amount_of_adiabatic_parts);
                            fprintf(fid,'*        rough   dhy                      vol.no.\n');
                            fprintf(fid,'1210801  0.      0.                       %s \n', amount_of_adiabatic_parts);
                            fprintf(fid,'*        tlpvbfe                          vol.no.\n');
                            fprintf(fid,'1211001  0001000                          %s \n', amount_of_adiabatic_parts);
                            fprintf(fid,'*        efvcahs                          jun.no.\n');
                            fprintf(fid,'1211101  0100000                          %s \n', adiabatic_minus_one);   
%                                 fprintf(fid,'*        ebt  temperature  static_quality        vol.no.\n');
%                                 fprintf(fid,'1211201  001  %s.   %s     0.  0.  0.     %s \n',PrimaryTemp,NC_mole_fr, amount_of_adiabatic_parts); %****************************************************
%                                 fprintf(fid,'*        ebt  pressure  static_quality        vol.no.\n');
%                                 fprintf(fid,'1211201  002  %s.   %s     0.  0.  0.     %s \n',PrimaryPressure,NC_mole_fr, amount_of_adiabatic_parts); %****************************************************
%                                 fprintf(fid,'*        ebt  pressure temperature  stat_qual       vol.no.\n');
%                                 fprintf(fid,'1211201  004  %s.      %s.          %s  0. 0.   %s \n',PrimaryPressure, PrimaryTemp,NC_mole_fr, amount_of_adiabatic_parts); %***************************************************
%                                 fprintf(fid,'1211201  004  %s.      %s.          0.0  0. 0.   %s \n',PrimaryPressure, PrimaryTemp, amount_of_adiabatic_parts); %***************************************************
                            fprintf(fid,'*        ebt  pressure liq_int_en gas_int_en  gas_void NC_quality   vol.no.\n');
                            fprintf(fid,'1211201  006 %s. %s %s 1.0 %s %s \n',PrimaryPressure,uLsat,uVsat, NC_mole_fr, amount_of_adiabatic_parts); %***************************************************
%                                 fprintf(fid,'*        ebt  pressure liq_temp gas_temp  gas_void NC_quality   vol.no.\n');
%                                 fprintf(fid,'1211201  008 %s. %s. %s. 0.9 %s %s \n',PrimaryPressure, PrimaryTemp, PrimaryTemp,NC_mole_fr, amount_of_adiabatic_parts); %***************************************************
                            fprintf(fid,'*        mass flow (=1)\n');
                            ...
                            fprintf(fid,'*----------------------------------------------------------------\n');
                            fprintf(fid,'* component 122 - adiabatic to condenser junction\n');
                            fprintf(fid,'*        name   type\n');
                            fprintf(fid,'1220000  injun  sngljun\n');
                            fprintf(fid,'*        from       to         area   floss rloss  jefvcahs\n');
                            fprintf(fid,'1220101  121150002  125010001  %s     0.    0.     00000000\n', cross_section);
                            fprintf(fid,'*       junctionD  flooding  gasintercept     slope\n');
                            ...
                            fprintf(fid,'*        area                             vol.no.\n');
                            fprintf(fid,'1250101  %s                               %s \n', cross_section, amount_of_condensation_parts);
                            fprintf(fid,'*        length                           vol.no.\n');
                            ...
                            fprintf(fid,'*        efvcahs                          jun.no.\n');
                            fprintf(fid,'1251101  0100000                          %s \n', condensation_minus_one);   
...
%                                 fprintf(fid,'1251201  004  %s.      %s.          0.0  0. 0.   %s \n',PrimaryPressure, PrimaryTemp, amount_of_condensation_parts); %***************
                            fprintf(fid,'*        ebt  pressure liq_int_en gas_int_en  gas_void NC_quality   vol.no.\n');
                            fprintf(fid,'1251201  006 %s. %s %s 1.0 %s %s \n',PrimaryPressure,uLsat,uVsat, NC_mole_fr, amount_of_condensation_parts); %***************************************************
%                                 fprintf(fid,'*        ebt  pressure liq_temp gas_temp  gas_void NC_quality   vol.no.\n');
%                                 fprintf(fid,'1251201  008  %s.      %s.      %s.       0.9      %s           %s \n',PrimaryPressure, PrimaryTemp, PrimaryTemp,NC_mole_fr, amount_of_condensation_parts); %***************************************************
                            ...
                            fprintf(fid,'1600102     0.0           0.0             00000000\n');
                            ...
                            fprintf(fid,'*         left.vol   incr.  b.c  Surfcode  Surffactor      HS.no.\n');
                            fprintf(fid,'11101501  121010000  10000  101  1       %s         %s \n', pipe_part_length, amount_of_adiabatic_parts);
                            fprintf(fid,'*         right.vol  incr.  b.c  Surfcode  Surffactor      HS.no.\n');
                            ...
                            fprintf(fid,'*         left.vol   incr.  b.c  Surfcode  Surffactor      HS.no.\n');
                            fprintf(fid,'11111501  125010000  10000  101  1       %s         %s \n', pipe_part_length, amount_of_condensation_parts);
                            fprintf(fid,'*         right.vol  incr.  b.c  Surfcode  Surffactor      HS.no.\n');
                            ...
                            fprintf(fid,'20255501 0.   -%s  \n',Heat_flux);   % HERE DEFINE HEAT FLUX
