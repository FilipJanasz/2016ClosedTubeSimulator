...
%     Pps=[1.004];                %Initial pressure [Bar]
% %     NC=[0,0.1,0.5,0.9];    %Non condensable mole fr (quality in relap)
%     NC=[1-0.602];               %Non condensable mole fr (quality in relap)
%     Helium=[0];             %Mole fraction of Helium in NC mixture
%     
%     %Secondary side - initial and operating conditions ********************************************************************************
%     Pss=[1.48];
%     Superheat=[23];  %T_secondary_side = T_primary_side(Pressure_primary_side) - superheat
...
%     Power=[500]; % [W]
    ...
    condenser_horizontal_cells=7;  %XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
%% Pre-test calculations
    ...
    % num2str is important when writing values with fprintf
        condenser_starting_num=condenser_vol_no+10000*(amount_of_adiabatic_parts+1);        
        condenser_starting=num2str(condenser_starting_num);
        ...
        condenser_unit_radius_inner=num2str(condenser_unit_radius);
        condenser_unit_radius_outer=num2str(condenser_unit_radius+wall_thickness);
...
%                                 fprintf(fid,'%s  006 %s. %s %s 1.0 %s %s \n',num2str(current_tube_no+1201),PrimaryPressure,uLsat,uVsat, NC_mole_fr, amount_of_tube_parts); %***************************************************
                                fprintf(fid,'*CCC1201 ebt press temp stat_qual       vol.no.\n');
                                fprintf(fid,'%s       004 %s.   %s  %s  0. 0.   %s \n',num2str(current_tube_no+1201),PrimaryPressure, PrimaryTemp,NC_mole_fr,amount_of_tube_parts); %*****
                            ...
                            %% ********************** CONDENSER - ADIABATIC 
                            % this loop connets a heat structure to every
                            % vertical pipe
                            for n=1:condenser_horizontal_cells
                                left_vol=condenser_vol_no+(n-1)*1000000+10000;
                                left_vol_condenser_starting=condenser_starting_num+(n-1)*1000000;
                                
                                fprintf(fid,'* heat structure data - wall shielding primary flow from environment\n');
                                fprintf(fid,'*----------------------------------------------------------------\n');
                                fprintf(fid,'* heat structure 12x - ADIABATIC WALL LEFT\n');
                                fprintf(fid,'*1CCCG000 no.HS  no.m.p  geo  s.s.flag  left\n');
%                                 fprintf(fid,'%s  %s     5       2    0         %s \n', num2str(condenser_heatstr),amount_of_adiabatic_parts, radius_inner);
                                fprintf(fid,'%s  %s     5       2    0         %s \n', num2str(condenser_heatstr), amount_of_adiabatic_parts, condenser_unit_radius_inner);
                                fprintf(fid,'*1CCCG100 mesh flag  format\n');
                                fprintf(fid,'%s  0          1\n',num2str(condenser_heatstr+100));
                                fprintf(fid,'*1CCCG101 intvl   right.cord.\n');
%                                 fprintf(fid,'%s  4       %s \n',num2str(condenser_heatstr+101), radius_outer);
                                fprintf(fid,'%s  4       %s \n', num2str(condenser_heatstr+101), condenser_unit_radius_outer);
                                fprintf(fid,'*1CCCG201  comp    intvl\n');
                                fprintf(fid,'%s  001     4\n',num2str(condenser_heatstr+201));
                                fprintf(fid,'*1CCCG301 source  intvl\n');
                                fprintf(fid,'%s  0.0     4\n',num2str(condenser_heatstr+301)); 
                                fprintf(fid,'*1CCCG401  temp    no.m.p\n');
                                fprintf(fid,'%s  %s     5\n',num2str(condenser_heatstr+401),PrimaryTemp);                           %****************************************************
                                fprintf(fid,'*1CCCG501   left.vol   incr.  b.c  Surfcode  Surffactor      HS.no.\n');
                                fprintf(fid,'%s  %s  10000  101  1       %s         %s \n',num2str(condenser_heatstr+501),num2str(left_vol), unit_vertical_height, amount_of_adiabatic_parts);
                                fprintf(fid,'*1CCCG601   right.vol  incr.  b.c  Surfcode  Surffactor      HS.no.\n');
                                fprintf(fid,'%s  0          0      0    1       %s         %s \n',num2str(condenser_heatstr+601), unit_vertical_height, amount_of_adiabatic_parts);
                                fprintf(fid,'*1CCCG701   s.type     mult   dir.left     dir.right  HS.no.\n');
                                fprintf(fid,'%s  0          1.     1.           1.         %s \n',num2str(condenser_heatstr+701), amount_of_adiabatic_parts);
                                fprintf(fid,'*1CCCG801  dhy\n');
    %                             fprintf(fid,'11201801  0.0  20.  20.  0.  0.  0.  0.  1.  %s \n', amount_of_adiabatic_parts);
                                fprintf(fid,'%s  %s  20.  20.  0.  0.  0.  0.  1.  %s \n',num2str(condenser_heatstr+801),heat_tr_hydr_diam, amount_of_adiabatic_parts);
                                fprintf(fid,'*1CCCG901   dhy\n');
    %                             fprintf(fid,'11201901  0.0  20.  20.  0.  0.  0.  0.  1.  %s \n', amount_of_adiabatic_parts);
                                fprintf(fid,'%s  %s  20.  20.  0.  0.  0.  0.  1.  %s \n',num2str(condenser_heatstr+901),heat_tr_hydr_diam, amount_of_adiabatic_parts);
                                fprintf(fid,'*----------------------------------------------------------------\n');
                                fprintf(fid,'\n');                   
                                
                                condenser_heatstr=condenser_heatstr+10000;
                                
                                %********************** HEAT EXCHANGER LEFT
                                fprintf(fid,'* heat structure data - heat exchanger\n');
                                fprintf(fid,'*----------------------------------------------------------------\n');
                                fprintf(fid,'* heat structure 12x - CONDENSER WALL LEFT\n');
                                fprintf(fid,'*1CCCG000 no.HS  no.m.p  geo  s.s.flag  left\n');
%                                 fprintf(fid,'%s  %s     5       2    0         %s \n', num2str(condenser_heatstr),amount_of_coolant_water, radius_inner);
                                fprintf(fid,'%s  %s     5       2    0         %s \n', num2str(condenser_heatstr), amount_of_coolant_water, condenser_unit_radius_inner);
                                fprintf(fid,'*1CCCG100 mesh flag  format\n');
                                fprintf(fid,'%s  0          1\n',num2str(condenser_heatstr+100));
                                fprintf(fid,'*1CCCG101 intvl   right.cord.\n');
%                                 fprintf(fid,'%s  4       %s \n',num2str(condenser_heatstr+101), radius_outer);
                                ...
                                fprintf(fid,'%s  %s     5\n',num2str(condenser_heatstr+401),PrimaryTemp);                           %****************************************************
                                fprintf(fid,'*1CCCG501   left.vol   incr.  b.c  Surfcode  Surffactor      HS.no.\n');
                                fprintf(fid,'%s  %s  10000  101  1       %s         %s \n',num2str(condenser_heatstr+501), num2str(left_vol_condenser_starting), unit_vertical_height, amount_of_coolant_water);
                                fprintf(fid,'*1CCCG601   right.vol  incr.  b.c  Surfcode  Surffactor      HS.no.\n');
                                fprintf(fid,'%s  155010000  10000  101  1       %s         %s \n',num2str(condenser_heatstr+601), unit_vertical_height, amount_of_coolant_water);
                                fprintf(fid,'*1CCCG701   s.type     mult   dir.left     dir.right  HS.no.\n');
                                fprintf(fid,'%s  0          1.     1.           1.         %s \n',num2str(condenser_heatstr+701), amount_of_coolant_water);
                                fprintf(fid,'*1CCCG801  dhy\n');
    %                             fprintf(fid,'11201801  0.0  20.  20.  0.  0.  0.  0.  1.  %s \n', amount_of_adiabatic_parts);
                                fprintf(fid,'%s  %s  20.  20.  0.  0.  0.  0.  1.  %s \n',num2str(condenser_heatstr+801),heat_tr_hydr_diam, amount_of_coolant_water);
                                fprintf(fid,'*1CCCG901   dhy\n');
    %                             fprintf(fid,'11201901  0.0  20.  20.  0.  0.  0.  0.  1.  %s \n', amount_of_adiabatic_parts);
                                fprintf(fid,'%s  %s  20.  20.  0.  0.  0.  0.  1.  %s \n',num2str(condenser_heatstr+901),heat_tr_hydr_diam, amount_of_coolant_water);
                                fprintf(fid,'*----------------------------------------------------------------\n');
                                fprintf(fid,'\n'); 
