...
%     NC=[0,0.1,0.5,0.9];               %Non condensable mole fr (quality in relap)
    NC=[0.1];               %Non condensable mole fr (quality in relap)
    ...
    heater_tank_length=0.6;
    heater_init_water_level=0.2;
    heater_diam_inner=0.0837;
    heater_diam_outer=0.0889;
    
    %"nodaliyation" of geometry
    amount_of_heater_full=10;
    amount_of_heater_empty=20;
...
%% Pre-test calculations
    ...
    % num2str is important when writing values with fprintf
        heater_length_full=num2str(heater_init_water_level/amount_of_heater_full);
        heater_length_empty=num2str((heater_tank_length-heater_init_water_level)/amount_of_heater_empty);
        
        pipe_part_length=condenser_length/amount_of_tube_parts;
        ...
        condenser_unit_volume=condenser_unit_cross_section*pipe_part_length;
        ...
        pipe_part_length=num2str(pipe_part_length);
...
%                             fprintf(fid,'20800001  tmass \n');
                            ...
                            heater_empty_vol_no=111000000;
                            condenser_vol_no=125000000;
                            
                            for heater_volume_counter=1:str2num(amount_of_heater_full_parts)
                                heater_full_vol_no=heater_full_vol_no+10000;
                                fprintf(fid,'%s  tmassv %s \n', num2str(card_no), num2str(heater_full_vol_no));
                                card_no=card_no+1;                                
                            end
                            
                            for heater_volume_2_counter=1:str2num(amount_of_heater_empty_parts)
                                heater_empty_vol_no=heater_empty_vol_no+10000;
                                fprintf(fid,'%s  tmassv %s \n', num2str(card_no), num2str(heater_empty_vol_no));
                                card_no=card_no+1;
                            end
                            ...
                            fprintf(fid,'1100301  %s                               %s \n', heater_length_full, amount_of_heater_full_parts);
...
%                                 fprintf(fid,'1101201  004  %s.      %s.          0.01  0. 0.   %s \n',PrimaryPressure, PrimaryTemp, amount_of_heater_tube_parts); %***************************************************
                            ...
                            fprintf(fid,'*----------------------------------------------------------------\n');
                            fprintf(fid,'* component 119 - inlet junction\n');
                            fprintf(fid,'*        name   type\n');
                            fprintf(fid,'1190000  injun  sngljun\n');
                            fprintf(fid,'*        from       to         area  floss rloss  jefvcahs\n');
                            fprintf(fid,'1190101  %s  111010001  %s    0.    0.     00000000\n', num2str(heater_full_vol_no+2), heater_cross_section);    %*************************************************************
                            fprintf(fid,'*       junctionD  flooding  gasintercept     slope\n');
                            fprintf(fid,'1190110  0.         0.        1.               1.\n');
                            fprintf(fid,'*        ctl  velflowf  velflowg     interface velocity\n');
                            fprintf(fid,'1190201  0.   0.       0.             0.\n');             %*********************
                            
                            fprintf(fid,'*----------------------------------------------------------------\n');
                            fprintf(fid,'* component 111 - empty heater\n');
                            fprintf(fid,'*        name   type\n');
                            fprintf(fid,'1110000  pipe  pipe\n');
                            fprintf(fid,'*        no.volumes\n');
                            fprintf(fid,'1110001  %s \n', amount_of_heater_empty_parts);  
                            fprintf(fid,'*        area                             vol.no.\n');
                            fprintf(fid,'1110101  %s                               %s \n', heater_cross_section, amount_of_heater_empty_parts);
                            fprintf(fid,'*        length                           vol.no.\n');
                            fprintf(fid,'1110301  %s                               %s \n', heater_length_empty, amount_of_heater_empty_parts);
                            fprintf(fid,'*        v-ang                            vol.no.\n');
                            fprintf(fid,'1110601  90.                              %s \n', amount_of_heater_empty_parts);
                            fprintf(fid,'*        rough   dhy                      vol.no.\n');
                            fprintf(fid,'1110801  0.      0.                       %s \n', amount_of_heater_empty_parts);
                            fprintf(fid,'*        tlpvbfe                          vol.no.\n');
                            fprintf(fid,'1111001  0001000                          %s \n', amount_of_heater_empty_parts);
                            fprintf(fid,'*        efvcahs                          jun.no.\n');
                            fprintf(fid,'1111101  0000000                          %s \n', heater_empty_minus_one);              
%                                 fprintf(fid,'*        ebt  temperature  stat_qual       vol.no.\n');
%                                 fprintf(fid,'1101201  001  %s.      %s          0.  0. 0.   %s \n', PrimaryTemp,NC_mole_fr, amount_of_heater_tube_parts); %***************************************************
%                                 fprintf(fid,'*        ebt  pressure  stat_qual         vol.no.\n');
%                                 fprintf(fid,'1101201  002  %s.   0.     0.  0.  0.     %s \n',PrimaryPressure, amount_of_heater_tube_parts); %****************************************************
                            fprintf(fid,'*        ebt  pressure temperature        vol.no.\n');
                            fprintf(fid,'1111201  003  %s.      %s          0.  0. 0.   %s \n',PrimaryPressure, num2str(Tps), amount_of_heater_empty_parts); %***************************************************
%                                 fprintf(fid,'*        ebt  pressure temperature  stat_qual       vol.no.\n');
%                                 fprintf(fid,'1101201  004  %s.      %s.          0.01  0. 0.   %s \n',PrimaryPressure, PrimaryTemp, amount_of_heater_tube_parts); %***************************************************
                            fprintf(fid,'*        mass flow (=1)\n');
                            fprintf(fid,'1111300  1\n');
                            fprintf(fid,'*        flowf  flowg      velj           jun.no.\n');
                            fprintf(fid,'1111301  0.     0.         0.             %s \n', heater_empty_minus_one);  %************
                            
                            fprintf(fid,'*----------------------------------------------------------------\n');
                            fprintf(fid,'* component 120 - inlet junction - left wall tube part\n');
                            fprintf(fid,'*        name   type\n');
                            fprintf(fid,'1200000  injun  sngljun\n');
                            fprintf(fid,'*        from       to         area  floss rloss  jefvcahs\n');
                            fprintf(fid,'1200101  %s  125010001  %s    0.    0.     00000100\n', num2str(heater_empty_vol_no+2), condenser_unit_cross_section);    %*************************************************************
                            fprintf(fid,'*       junctionD  flooding  gasintercept     slope\n');
                            ...
                            fprintf(fid,'*----------------------------------------------------------------\n');
                            fprintf(fid,'* component 121 - inlet junction - center tube part\n');
                            fprintf(fid,'*        name   type\n');
                            fprintf(fid,'1210000  injun  sngljun\n');
                            fprintf(fid,'*        from       to         area  floss rloss  jefvcahs\n');
                            fprintf(fid,'1210101  %s  126010001  %s    0.    0.     00000100\n', num2str(heater_empty_vol_no+2), condenser_unit_cross_section);    %*************************************************************
                            fprintf(fid,'*       junctionD  flooding  gasintercept     slope\n');
                            fprintf(fid,'1210110  0.         0.        1.               1.\n');
                            fprintf(fid,'*        ctl  velflowf  velflowg     interface velocity\n');
                            fprintf(fid,'1210201  0.   0.       0.             0.\n');             %***********
                            
                             fprintf(fid,'*----------------------------------------------------------------\n');
                            fprintf(fid,'* component 122 - inlet junction - right wall tube part\n');
                            fprintf(fid,'*        name   type\n');
                            fprintf(fid,'1220000  injun  sngljun\n');
                            fprintf(fid,'*        from       to         area  floss rloss  jefvcahs\n');
                            fprintf(fid,'1220101  %s  127010001  %s    0.    0.     00000100\n', num2str(heater_empty_vol_no+2), condenser_unit_cross_section);    %*************************************************************
                            fprintf(fid,'*       junctionD  flooding  gasintercept     slope\n');
                            fprintf(fid,'1220110  0.         0.        1.               1.\n');
                            fprintf(fid,'*        ctl  velflowf  velflowg     interface velocity\n');
                            fprintf(fid,'1220201  0.   0.       0.             0.\n');             %***********
                            
                            %******************** tube (adiabatic part + condenser)
                            fprintf(fid,'*----------------------------------------------------------------\n');
                            fprintf(fid,'* component 125 - heat exchanger part of the pipe - wall\n');
                            fprintf(fid,'*        name   type\n');
                            fprintf(fid,'1250000  pipe  pipe\n');
                            fprintf(fid,'*        no.volumes\n');
                            fprintf(fid,'1250001  %s \n', amount_of_tube_parts);  
                            fprintf(fid,'*        area vert                        vol.no.\n');
                            fprintf(fid,'1250101  %s                               %s \n', condenser_unit_cross_section, amount_of_tube_parts);
                            fprintf(fid,'*        area horz                        vol.no.\n');
                            fprintf(fid,'1251601  %s                               %s \n', condenser_unit_side_area, amount_of_tube_parts);
                            fprintf(fid,'*        length vert                      vol.no.\n');
                            fprintf(fid,'1250301  %s                               %s \n', pipe_part_length, amount_of_tube_parts);
%                             fprintf(fid,'*        length horz                      vol.no.\n');
%                             fprintf(fid,'1251801  %s                               %s \n', condenser_unit_diam, amount_of_tube_parts);
                            fprintf(fid,'*        v-ang                            vol.no.\n');
                            fprintf(fid,'1250601  90.                              %s \n', amount_of_tube_parts);
                            fprintf(fid,'*        rough   hydrD vert               vol.no.\n');
                            fprintf(fid,'1250801  0.      0.                       %s \n', amount_of_tube_parts);
                            fprintf(fid,'*        rough   hydrD horz               vol.no.\n');
                            fprintf(fid,'1252301  0.      0.                       %s \n', amount_of_tube_parts);
                            fprintf(fid,'*        tlpvbfe                          vol.no.\n');
...
%                                 fprintf(fid,'1251201  008  %s.      %s.      %s.       0.9      %s           %s \n',PrimaryPressure, PrimaryTemp, PrimaryTemp,NC_mole_fr, amount_of_condensation_parts); %***************************************************
                            ...
                            fprintf(fid,'*----------------------------------------------------------------\n');
                            fprintf(fid,'* component 126 - heat exchanger part of the pipe - center\n');
                            fprintf(fid,'*        name   type\n');
                            fprintf(fid,'1260000  pipe  pipe\n');
                            fprintf(fid,'*        no.volumes\n');
                            fprintf(fid,'1260001  %s \n', amount_of_tube_parts);  
                            fprintf(fid,'*        area vert                        vol.no.\n');
                            fprintf(fid,'1260101  %s                               %s \n', condenser_unit_cross_section, amount_of_tube_parts);
                            fprintf(fid,'*        area horz                        vol.no.\n');
                            fprintf(fid,'1261601  %s                               %s \n', condenser_unit_side_area, amount_of_tube_parts);
                            fprintf(fid,'*        length vert                      vol.no.\n');
                            fprintf(fid,'1260301  %s                               %s \n', pipe_part_length, amount_of_tube_parts);
%                             fprintf(fid,'*        length horz                      vol.no.\n');
%                             fprintf(fid,'1261801  %s                               %s \n', condenser_unit_diam, amount_of_tube_parts);
                            fprintf(fid,'*        v-ang                            vol.no.\n');
                            fprintf(fid,'1260601  90.                              %s \n', amount_of_tube_parts);
                            fprintf(fid,'*        rough   hydrD vert               vol.no.\n');
                            fprintf(fid,'1260801  0.      0.                       %s \n', amount_of_tube_parts);
                            fprintf(fid,'*        rough   hydrD horz               vol.no.\n');
                            fprintf(fid,'1262301  0.      0.                       %s \n', amount_of_tube_parts);
                            fprintf(fid,'*        tlpvbfe                          vol.no.\n');
                            fprintf(fid,'1261001  0001000                          %s \n', amount_of_tube_parts);
                            fprintf(fid,'*        efvcahs                          jun.no.\n');
                            fprintf(fid,'1261101  0000000                          %s \n', tube_minus_one);   
                            fprintf(fid,'*        ebt  pressure liq_int_en gas_int_en  gas_void NC_quality   vol.no.\n');
                            fprintf(fid,'1261201  006 %s. %s %s 1.0 %s %s \n',PrimaryPressure,uLsat,uVsat, NC_mole_fr, amount_of_tube_parts); %***************************************************
                            fprintf(fid,'*        mass flow (=1)\n');
                            fprintf(fid,'1261300  1\n');
                            fprintf(fid,'*        flowf  flowg      velj           jun.no.\n');
                            fprintf(fid,'1261301  0.     0.         0.             %s \n', tube_minus_one);  %***************************
                            
                                                        %******************** tube (adiabatic part + condenser)
                            fprintf(fid,'*----------------------------------------------------------------\n');
                            fprintf(fid,'* component 126 - heat exchanger part of the pipe - center\n');
                            fprintf(fid,'*        name   type\n');
                            fprintf(fid,'1270000  pipe  pipe\n');
                            fprintf(fid,'*        no.volumes\n');
                            fprintf(fid,'1270001  %s \n', amount_of_tube_parts);  
                            fprintf(fid,'*        area vert                        vol.no.\n');
                            fprintf(fid,'1270101  %s                               %s \n', condenser_unit_cross_section, amount_of_tube_parts);
                            fprintf(fid,'*        area horz                        vol.no.\n');
                            fprintf(fid,'1271601  %s                               %s \n', condenser_unit_side_area, amount_of_tube_parts);
                            fprintf(fid,'*        length vert                      vol.no.\n');
                            fprintf(fid,'1270301  %s                               %s \n', pipe_part_length, amount_of_tube_parts);
%                             fprintf(fid,'*        length horz                      vol.no.\n');
%                             fprintf(fid,'1271801  %s                               %s \n', condenser_unit_diam, amount_of_tube_parts);
                            fprintf(fid,'*        v-ang                            vol.no.\n');
                            fprintf(fid,'1270601  90.                              %s \n', amount_of_tube_parts);
                            fprintf(fid,'*        rough   hydrD vert               vol.no.\n');
                            fprintf(fid,'1270801  0.      0.                       %s \n', amount_of_tube_parts);
                            fprintf(fid,'*        rough   hydrD horz               vol.no.\n');
                            fprintf(fid,'1272301  0.      0.                       %s \n', amount_of_tube_parts);
                            fprintf(fid,'*        tlpvbfe                          vol.no.\n');
                            fprintf(fid,'1271001  0001000                          %s \n', amount_of_tube_parts);
                            fprintf(fid,'*        efvcahs                          jun.no.\n');
                            fprintf(fid,'1271101  0000000                          %s \n', tube_minus_one);   
                            fprintf(fid,'*        ebt  pressure liq_int_en gas_int_en  gas_void NC_quality   vol.no.\n');
                            fprintf(fid,'1271201  006 %s. %s %s 1.0 %s %s \n',PrimaryPressure,uLsat,uVsat, NC_mole_fr, amount_of_tube_parts); %***************************************************
                            fprintf(fid,'*        mass flow (=1)\n');
                            fprintf(fid,'1271300  1\n');
                            fprintf(fid,'*        flowf  flowg      velj           jun.no.\n');
                            fprintf(fid,'1271301  0.     0.         0.             %s \n', tube_minus_one);  %***************************
                            
                            %horizontal junction 1
                            fprintf(fid,'*----------------------------------------------------------------\n');
                            fprintf(fid,'* component 130 - crossflow junction\n');
                            fprintf(fid,'*        name   type\n');
                            ...
                            fprintf(fid,'1300011  125010004 126010003 %s 1. 1. 0000010 1. 1. 1. 10000 10000 0 %s\n',condenser_unit_side_area, amount_of_tube_parts); %, condeser_unit_side_area);    %*************************************************************
                            ...
                            fprintf(fid,'*----------------------------------------------------------------\n');
                            fprintf(fid,'* component 131 - crossflow junction\n');
                            fprintf(fid,'*        name   type\n');
                            fprintf(fid,'1310000  horzjun  mtpljun\n');
                            fprintf(fid,'*        no_of_jun   init_cond_ctrl\n');
                            fprintf(fid,'1310001  %s          0\n', amount_of_tube_parts);
                            fprintf(fid,'*        from      to        area  floss rloss  efvcahs  W7 W8 W9  from_incr to_incr  W12  junction_lim\n');
                            fprintf(fid,'1310011  126010004 127010003 %s 1. 1. 0000010 1. 1. 1. 10000 10000 0 %s\n',condenser_unit_side_area, amount_of_tube_parts); %, condeser_unit_side_area);    %*************************************************************
                            fprintf(fid,'*        initflow_f initflow_g  junction_lim\n');
                            fprintf(fid,'1311011  0.         0.          %s\n',amount_of_tube_parts);
                            fprintf(fid,'*        hydrD  CCFL gas_intercept slope junction_lim\n');
                            fprintf(fid,'1312011  0.     0.   1.            1.    %s\n',amount_of_tube_parts);            %**
                            ...
                            fprintf(fid,'11051601  0          0      2555 1       %s        %s \n', heater_length_empty, amount_of_heater_full_parts);
                            ...
                            fprintf(fid,'*         left.vol   incr.  b.c  Surfcode  Surffactor      HS.no.\n');
                            fprintf(fid,'11061501  111010000  10000  101  1       %s        %s \n', heater_length_empty, amount_of_heater_empty_parts);
                            fprintf(fid,'*         right.vol  incr.  b.c  Surfcode  Surffactor      HS.no.\n');
